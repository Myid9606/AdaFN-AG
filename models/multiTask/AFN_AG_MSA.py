...
class StyleRandomization(nn.Module):
    ...
    def forward(self, x, aug_mean, aug_var, K = 1):
        ...
        if self.training:
            x = x.view(N, C, -1)
            mean = x.mean(-1, keepdim=True)  #计算得到x的均值和方差
            ...
            if x.is_cuda:
                alpha = alpha.cuda()
            mean = (1 - alpha) * mean + alpha * aug_mean #将辅助的均值和方差与原始的进行加权混合
...
class AFN_AG_MSA(nn.Module):
    def __init__(self, args):
        ...
        self.SAG_Transformer_1 = SAG_Transformer(hidden_dim=self.hidden_dim)    #添加了门控机制的跨模态交互Transformer
        self.SAG_Transformer_2 = SAG_Transformer(hidden_dim=self.hidden_dim)
        self.audio_adaIN = StyleRandomization()                                #添加了Adaptive Instance Norm
    ...
    def forward(self, text, audio, video):
        ...
        ####################Attention Gate222222#################
        # 消融a注释audio、text，消融v注释video、text，不消融不注释
        audio, text_audio_hidden = self.SAG_Transformer_1(text, audio)#调用SAG模块，实现音频与文本的跨模态交互
        video, text_video_hidden = self.SAG_Transformer_2(text, video)#调用SAG模块，实现视频与文本的跨模态交互


        text = torch.cat([text_audio_hidden, text_video_hidden], dim=1)
        #############Attention Gate2222222#################
        text = text.mean(dim=1)    # [batch, len, dim] -> [batch, dim]
...
class AuViSubNet(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        y_1 = self.linear_1(h)       # [batch, sequence_len, out_dim]
        return y_1
...
class SAG_Transformer(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.tgt_self_attention = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=2, batch_first=True, dropout=0.1)
    ...
    def forward(self, text, target):
        ...
        text_hidden, _ = self.text_gru(text) #首先对输入进行gru编码，
        text_retrain_gate = self.sigmoid(self.text_retrain_gate(text_hidden))#对gru编码后的结果，用一个Linear层映射到一维后并经过sigmoid得到门控分数
        text_compound_gate = self.sigmoid(self.text_compound_gate(text_hidden)) #对gru编码后的结果，用一个Linear层映射到一维后并经过sigmoid得到门控分数
        text_attention = self.text_self_attention(text_hidden, text_hidden, text_hidden)[0]#经过self-attention
        new_text_hidden = self.norm(text_retrain_gate * text_hidden + text_compound_gate * text_attention)#利用得到的门控分数控制不同变量的权重
        ...
        tgt_cross_attention = self.cross_attention(target_attention, text_attention, text_attention)[0]#让文本的特征与目标特征进行cross attention
        new_tgt_hidden = self.norm(tgt_retrain_gate * tgt_hidden + tgt_compound_gate * tgt_cross_attention) #利用得到的门控分数控制不同变量的权重
