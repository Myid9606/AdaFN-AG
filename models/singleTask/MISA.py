...
class MISA(nn.Module):
    ...
    def alignment(self, text, acoustic, visual):
        ...
        # *********************************Ada IN*******************************
        audio_dim = acoustic.size(2)
        visual_dim = visual.size(2)
        visual_mean = visual.permute(0, 2, 1).mean(-1, keepdim=True)
        visual_mean = visual_mean.mean(1).unsqueeze(1).repeat((1, audio_dim, 1))
        visual_var = visual.permute(0, 2, 1).var(-1, keepdim=True)
        visual_var = visual_var.mean(1).unsqueeze(1).repeat((1, audio_dim, 1))
        acoustic_mean = acoustic.permute(0, 2, 1).mean(-1, keepdim=True)
        acoustic_mean = acoustic_mean.mean(1).unsqueeze(1).repeat((1, visual_dim, 1))

        acoustic_var = acoustic.permute(0, 2, 1).var(-1, keepdim=True)
        acoustic_var = acoustic_var.mean(1).unsqueeze(1).repeat((1, visual_dim, 1))
        visual = self.video_adaIN(visual, acoustic_mean, acoustic_var, K=5)
        acoustic = self.audio_adaIN(acoustic, visual_mean, visual_var, K=5)
        # *********************************Ada IN*******************************
        ...
        #print("utterance_text:", utterance_text.size())
