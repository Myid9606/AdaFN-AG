...
class MMIM(nn.Module):
    ...
    def forward(self, text, audio, vision, y=None, mem=None):
        ...
        visual_dim = vision.size(2)
        visual_mean = vision.permute(0, 2, 1).mean(-1, keepdim=True)
        visual_mean = visual_mean.mean(1).unsqueeze(1).repeat((1, audio_dim, 1))
        visual_var = vision.permute(0, 2, 1).var(-1, keepdim=True)
        ...
        vision = self.video_adaIN(vision, acoustic_mean, acoustic_var, K=8)#可以调节k，可以单行注释
        audio = self.audio_adaIN(audio, visual_mean, visual_var, K=5) #可以调节k，可以单行注释
        # *********************************Ada IN*******************************
